...
class Hangman:
    ...
    def __init__(self,wordlist):
        ...
        self.attems_remaining = 6
        self.current_letter = ''
        self.chosen_word = ''
        self.gussed_latter = []

    def chose_the_word(self):
        file = open(self.wordlist)
        words = file.read().split('\n')
        words_count = len(words)
        self.chosen_word = words[random.randrange(0,words_count)]
    ...
    def guess_the_letter(self):
        ...
        if(letter in self.gussed_latter):
            print("You already gussed that letter Your Gusees is {}.",format(''.join(self.gussed_latter)))
            return

        self.gussed_latter.append(letter)
        occurences = []
        ...
        if(len(occurences) == 0):
            self.attems_remaining -= 1
            print("Oops Your Guess Was Wrong. Attemps remaining is {}.",format(self.attems_remaining))
